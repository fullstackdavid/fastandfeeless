...
axios
  .all([getOnlineReps(), getConfirmationQuorum(), getCirculatingSupply()])
  .then(
    axios.spread((reps, quorum, supply) => {
      // Both requests are now complete
      reps_online = Object.keys(reps.data.representatives).length;
      voting_weight_online = quorum.data.online_stake_total;
      voting_weight_online =
        voting_weight_online / 1000000000000000000000000000000000000;
      ...
      percentVotingPower =
        Math.round((voting_weight_online / cirsupply) * 100 * 100) / 100;
      twitterClient
        .post("statuses/update", {
          status:
            "Number of representatives online : " +
            reps_online +
            ...
            cirsupply +
            "M $NANO" +
            "\r\n\n\tPercentage of voting power online : " +
            percentVotingPower +
            "%",
        })
        .then(function (tweet) {
